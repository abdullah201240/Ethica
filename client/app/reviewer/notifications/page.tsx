...
import { getActiveReviewer } from "@/lib/reviewer-roster"
import {
...
export default function ReviewerNotificationsPage() {
  ...
  const [currentReviewer, setCurrentReviewer] = React.useState(getActiveReviewer)
  const [notifications, setNotifications] = React.useState<EthicaNotification[]>(() => {
    const reviewer = getActiveReviewer()
    return getNotificationsForRole("reviewer", reviewer.email)
  })
  ...
  const syncData = React.useCallback(() => {
    ...
    const list = getNotificationsForRole("reviewer", reviewer.email)
  ...
  const unreadCount = getUnreadCountForRole("reviewer", currentReviewer.email)
  const urgentCount = notifications.filter(
    (n) => n.priority === "urgent" || n.priority === "high"
  ).length
  const invitationCount = notifications.filter(
    (n) => n.category === "deliberation" && (n.title.includes("Assignment") || n.title.includes("Request"))
  ).length
  ...
  const handleMarkAllAsRead = () => {
    markAllNotificationsAsRead("reviewer", currentReviewer.email)
    syncData()
    toast.success("All reviewer deliberation notices marked as read")
  }

  const handleClearRead = () => {
    clearReadNotifications("reviewer", currentReviewer.email)
    syncData()
    toast.success("Read notices cleared from deliberation queue")
  }
  ...
  const handleToggleRead = (id: string, currentRead: boolean) => {
    if (!currentRead) {
      ...
      syncData()
      toast.success("Deliberation notice marked as read")
    }
  }
  ...
  const handleSimulateAlert = () => {
    const notif = simulateIncomingAlert("reviewer")
    syncData()
    ...
    <div className="w-full space-y-6 pb-12 select-text">
      ...
      <KpiGrid columns={4}>
        ...
        <KpiCard
          ...
          badge={
            unreadCount > 0 ? (
              <Badge className="bg-amber-600 text-white text-xs font-bold rounded-md">
                {unreadCount} New
              </Badge>
            ) : undefined
          }
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-[240px]">
            ...
            <Input
              ...
              placeholder="Search deliberation notices by protocol, investigator, or topic..."
              ...
              <AlertDialogContent className="w-full max-w-[calc(100vw-2rem)] sm:max-w-md">
                <AlertDialogHeader>
                  <AlertDialogTitle className="text-lg font-bold text-foreground">
                    Purge Read Deliberation Notices?
                  </AlertDialogTitle>
                  <AlertDialogDescription className="text-body-sm text-muted-foreground">
                    This will clear all acknowledged notices from your committee reviewer notifications list. Ongoing deliberation assignments remain in your workspace.
                  </AlertDialogDescription>
                </AlertDialogHeader>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <Select value={selectedCategory} onValueChange={(val) => setSelectedCategory(val ?? "all")}>
              ...
              <SelectContent>
                ...
                <SelectItem value="deliberation">Peer Deliberations</SelectItem>
                <SelectItem value="protocol">Protocol Amendments</SelectItem>
                <SelectItem value="accreditation">Accreditation Dossier</SelectItem>
              </SelectContent>
            </Select>
        ...
        {filteredNotifications.length === 0 ? (
          <div className="rounded-xl border border-border/75 bg-white dark:bg-[#0C1E34] p-12 text-center flex flex-col items-center justify-center">
            <div className="size-14 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-muted-foreground mb-3 shadow-xs">
              <Scale className="size-7 text-slate-400" />
            </div>
            <h3 className="text-lg font-bold text-foreground">
              No deliberation notices found
            </h3>
            <p className="text-body-sm text-muted-foreground mt-1 max-w-md">
              {searchQuery || selectedCategory !== "all" || selectedPriority !== "all"
                ? "Try adjusting your query or resetting filter selectors."
                : "Your committee reviewer inbox is completely up to date."}
            </p>
          </div>
        ) : (
          filteredNotifications.map((notif) => (
            <div
              ...
              className={cn(
                ...
                !notif.read
                  ? "bg-white dark:bg-[#0C1E34] border-amber-300 dark:border-amber-800 ring-1 ring-amber-500/10"
              ...
              <div className="flex items-start gap-3.5 min-w-0 flex-1">
                ...
                >
                  {notif.category === "deliberation" ? (
                    ...
                    <FileCheck className="size-5 text-sky-600 dark:text-sky-400" />
                  ) : (
                ...
                <div className="space-y-1.5 min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    {!notif.read && (
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-800 dark:text-amber-300 bg-amber-100 dark:bg-amber-950 px-2 py-0.5 rounded-full">
              ...
              <div className="flex items-center gap-2 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-border/40">
                ...
                {notif.actionUrl && (
                  <Link href={notif.actionUrl}>
                    ...
                    >
                      <span>{notif.actionLabel || "Deliberate"}</span>
            ...
            <>
              <SheetHeader className="space-y-2 text-left">
                ...
                <SheetDescription className="text-xs text-muted-foreground">
                  Committee Deliberation Docket: {inspectingNotif.id}
                </SheetDescription>
              </SheetHeader>
              ...
              <div className="rounded-xl border border-border/75 p-4 space-y-3 bg-white dark:bg-[#0C1E34]">
                <h5 className="text-xs font-bold uppercase tracking-wider text-slate-500">
                  Committee Reviewer Dossier
                </h5>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-muted-foreground block">Appointed Reviewer</span>
                    <span className="font-semibold text-foreground">
                      {currentReviewer.name}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block">Board Panel</span>
                    <span className="font-semibold text-foreground">
                      {currentReviewer.board}
                    </span>
                  </div>
                  <div className="col-span-2">
                    <span className="text-muted-foreground block">Accreditation Seal</span>
                ...
                <div className="pt-2 border-t border-border/50 flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400 font-semibold">
                  ...
                  <span>Verified Peer Deliberation Chamber Action</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                {inspectingNotif.actionUrl && (
                  ...
                  >
                    ...
                    >
                      <span>{inspectingNotif.actionLabel || "Open Deliberation"}</span>
